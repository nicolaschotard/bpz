...
pars.d = {
    ...
    # Save all the galaxy probability distributions (it will create a very large file)
    'PROBS': 'no',
    # Save all the galaxy probability distributions P(z,t) (but not priors) -- Compact
    'PROBS2': 'no',
...
if pars.d['MAG'] == 'yes':
    ...
    ef_obs = where(greater_equal(ef_obs, 0.), clip(
...
ef_obs = where(less(f_obs, 0.), maximum(1e-100, f_obs + ef_obs),
...
ef_obs = where(no_seen, sqrt(ef_obs * ef_obs +
...
if pars.d['ZC']:
    ...
    if type(pars.d['ZC']) == type(""):
        ...
        zc = array(list(map(float, pars.d['ZC'])))
    if type(pars.d['FC']) == type(""):
        fc = array([float(pars.d['FC'])])
    else:
        fc = array(list(map(float, pars.d['FC'])))
...
for ig in range(ng):
    ...
    if pars.d['N_PEAKS'] > 1:
        ...
        g_max = less(p_bayes[2:], p_bayes[1:-1]) * \
            less(p_bayes[:-2], p_bayes[1:-1])
        g_min = greater(p_bayes[2:], p_bayes[1:-1]) * \
            greater(p_bayes[:-2], p_bayes[1:-1])
    ...
    if checkSED:
        ...
        flux_comparison = [id[ig], m_0[ig], z[iz_b],
                           t_b, am] + list(concatenate([ft, fo, efo]))
...
if plots and checkSED:
    ...
    if 'Z_S' in col_pars.d:
        if not interactive or ask('Compare z_B vs z_spec?'):
            good = less(z_s, 9.99)
            print(
                'Total initial number of objects with spectroscopic redshifts= ', sum(good))
            od_th = 0.
            if ask('Select for galaxy characteristics?\n'):
                ...
                if has_mags:
                    ...
                    good = good * less_equal(m_0, mg_max) * \
                        greater_equal(m_0, mg_min)
